...
//
// IT IS CUT TO ROTATION-ONLY HERE, NOT AT LOAD. A position track bakes the ALIEN's bone
// lengths, and pouring those into Colin stretches him to alien proportions -- the same reason
// `skinClips` keeps only quaternions. Doing it offline makes the file two thirds smaller and
// means no clip in it can ever be applied the wrong way by accident.
...
let chDrop = 0, sDrop = 0;
for (const a of root.listAnimations()) {
  for (const c of a.listChannels()) if (c.getTargetPath() !== 'rotation') { c.dispose(); chDrop++; }
...
console.log('kept ' + kept.length + ' clips (' + kept.join(', ') + '), dropped ' + dropped + ' clips, ' + chDrop + ' position/scale channels and ' + sDrop + ' samplers');
